	...
	useEffect(() => {
		...
		return mergeRegister(
			...
			editor.registerCommand(
				...
				() => {
					editor.update(() => {
						const x = $createCollapsibleTitleNode();
						const selection = $getSelection();
						if (
							$isRangeSelection(selection)
						) {
							...
							if ($isRootOrShadowRoot(anchorNodeParent)) {
								...
								const coll = $createCollapsibleContainerNode(true).append(
									x,
									contentNode
								)
								append(contentNode, parent.getChildren());
								parent.replace(coll);
								x.select();
							}
						}
