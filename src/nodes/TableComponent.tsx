...
import { IS_APPLE } from './../../packages/shared/src/environment';
	...
	return (
		<CellComponent
			className={`${theme.tableCell} ${
				isHeader ? theme.tableCellHeader : ''
	...
	useEffect(() => {
		...
		const handlePointerUp = (event: PointerEvent) => {
			if (resizingID !== null) {
				...
				updateTableNode((tableNode) => {
					const [x] = cellCoordMap.get(resizingID) as [
						number,
						number,
					];
					$addUpdateTag('history-push');
		...
		if (!isEditing && primarySelectedCellID !== null) {
			...
			const loadContentIntoCell = (cell: Cell | null) => {
				if (cell !== null && cellEditor !== null) {
					const editorStateJSON = cell.json;
					const editorState =
						cellEditor.parseEditorState(editorStateJSON);
					cellEditor.setEditorState(editorState);
				}
			};
			...
			const handleDblClick = (event: MouseEvent) => {
				const possibleID = getCellID(event.target as HTMLElement);
				if (
					possibleID === primarySelectedCellID &&
					editor.isEditable()
				) {
				...
				if (htmlString) {
					try {
						...
						if (possibleTableElement != null) {
							const pasteRows =
								extractRowsFromHTML(possibleTableElement);
							updateTableNode((tableNode) => {
								const [x, y] = cellCoordMap.get(
									primarySelectedCellID
								) as [number, number];
								$addUpdateTag('history-push');
				...
				(event, targetEditor) => {
					...
					if (!isEditing && selection === null) {
						...
						const cellID = extend
							...
							: primarySelectedCellID;
						if (cellID !== null) {
							const [x, y] = cellCoordMap.get(cellID) as [
								number,
								number,
							];
							if (y !== 0) {
				...
				(event, targetEditor) => {
					...
					if (selection.isCollapsed()) {
						...
						if (
							...
							(anchor.type === 'element' &&
								anchor.offset ===
									anchor.getNode().getChildrenSize())
						) {
							event.preventDefault();
							return true;
						}
		...
		>
			<table
				className={`${theme.table} ${
					isSelected ? theme.tableSelected : ''
				}`}
				ref={tableRef}
				tabIndex={-1}
			>
